...
import { handleProcessProveRequest } from '../Offscreen/rpc';
...
export async function handleFinishProveRequest(
  request: BackgroundAction,
  sendResponse: (data?: any) => void,
) {
...
async function handleRetryProveReqest(
  ...
  if (chrome && (chrome as any).offscreen) {
    ...
    await handleProcessProveRequest(r, () => null);
  }
...
async function handleProveRequestStart(
  ...
  if (chrome && (chrome as any).offscreen) {
    ...
    await handleProcessProveRequest(req, () => null);
  }
